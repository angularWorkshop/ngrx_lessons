...
describe("Оптимизировать CourseList так, чтобы изменение selectedId не пересоздавало все карточки.", () => {
  it('keeps the NgRx exercise contract explicit', () => {
    expect(exerciseContract.course).toBe('NGRX');
    expect(exerciseContract.topic).toBe("Performance: memoization, selector granularity и change detection");
    expect(exerciseContract.exercise).toBe("Оптимизировать CourseList так, чтобы изменение selectedId не пересоздавало все карточки.");
  ...
  it('starts from a draft plan before implementation', () => {
    expect(createNgrxExercisePlan()).toEqual({
      ...
      exercise: "Оптимизировать CourseList так, чтобы изменение selectedId не пересоздавало все карточки.",
      feature: "performance memoization selector g",
      status: 'draft',
