...
describe("Убрать дублирующиеся actions и составить читабельные action groups по source.", () => {
  it('keeps the NgRx exercise contract explicit', () => {
    expect(exerciseContract.course).toBe('NGRX');
    expect(exerciseContract.topic).toBe("Action taxonomy: event storming для frontend-state");
    expect(exerciseContract.exercise).toBe("Убрать дублирующиеся actions и составить читабельные action groups по source.");
