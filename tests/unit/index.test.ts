...
describe("Создать CoursesFeature и базовые selectors для ids/entities/status/filter.", () => {
  it('keeps the NgRx exercise contract explicit', () => {
    expect(exerciseContract.course).toBe('NGRX');
    expect(exerciseContract.topic).toBe("createFeature и feature selectors");
    expect(exerciseContract.exercise).toBe("Создать CoursesFeature и базовые selectors для ids/entities/status/filter.");
