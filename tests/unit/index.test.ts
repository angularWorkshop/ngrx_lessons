...
describe("Добавить withHooks в CourseFiltersSignalStore для initial load и cleanup.", () => {
  it('keeps the NgRx exercise contract explicit', () => {
    ...
    expect(exerciseContract.exercise).toBe("Добавить withHooks в CourseFiltersSignalStore для initial load и cleanup.");
  ...
  it('starts from a draft plan before implementation', () => {
    expect(createNgrxExercisePlan()).toEqual({
      ...
      exercise: "Добавить withHooks в CourseFiltersSignalStore для initial load и cleanup.",
