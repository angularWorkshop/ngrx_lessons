...
describe("Добавить init/load action при подключении lazy feature и проверить, что effect стартует один раз.", () => {
  it('keeps the NgRx exercise contract explicit', () => {
    ...
    expect(exerciseContract.exercise).toBe("Добавить init/load action при подключении lazy feature и проверить, что effect стартует один раз.");
