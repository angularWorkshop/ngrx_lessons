...
describe("Переписать class-based loadCourses effect на functional effect с inject.", () => {
  it('keeps the NgRx exercise contract explicit', () => {
    expect(exerciseContract.course).toBe('NGRX');
    expect(exerciseContract.topic).toBe("Functional Effects и effect lifecycle");
    expect(exerciseContract.exercise).toBe("Переписать class-based loadCourses effect на functional effect с inject.");
  ...
  it('starts from a draft plan before implementation', () => {
    expect(createNgrxExercisePlan()).toEqual({
      ...
      exercise: "Переписать class-based loadCourses effect на functional effect с inject.",
      feature: "functional effects i effect lifecy",
      status: 'draft',
