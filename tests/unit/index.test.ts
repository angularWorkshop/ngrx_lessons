...
describe("Написать migration persisted state v1 → v2 и обработать невалидный localStorage.", () => {
  it('keeps the NgRx exercise contract explicit', () => {
    expect(exerciseContract.course).toBe('NGRX');
    expect(exerciseContract.topic).toBe("Persistence, SSR/hydration и миграции state shape");
    expect(exerciseContract.exercise).toBe("Написать migration persisted state v1 → v2 и обработать невалидный localStorage.");
